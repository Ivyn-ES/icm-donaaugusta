...
if (!user || user.permissoes !== 'grupo1') {
...
function carregarPresencasGrupo1() {
    ...
    membrosGrupo1.forEach(nome => {
        const presente = presencasGrupo1[data] ? presencasGrupo1[data][nome] || false : false;
        const div = document.createElement('div');
        div.style.cssText = 'margin:10px 0;padding:15px;background:#f0f8ff;border-radius:8px;display:flex;justify-content:space-between;';
        div.innerHTML = `
            <span>${nome}</span>
            <label><input type="checkbox" data-nome="${nome}" data-data="${data}" ${presente ? 'checked' : ''}> Presente</label>
        `;
...
function carregarRelatorioGrupo1() {
    ...
    for (let data in presencasGrupo1) {
        const dia = Object.values(presencasGrupo1[data]).filter(Boolean).length;
        total++;
        if (dia > 0) presente++;
    }
    ...
    relatorio.innerHTML = `
        <div style="background:#d4edda;padding:20px;border-radius:10px;">
            <h4>📊 Resumo Grupo 1</h4>
            <p>Total reuniões: ${total} | Presentes: ${presente} (${Math.round(presente/total*100)||0}%)</p>
        </div>
    `;
}

carregarPresencasGrupo1();
carregarRelatorioGrupo1();
